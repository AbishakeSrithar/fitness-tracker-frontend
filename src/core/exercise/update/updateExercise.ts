...
export function addEventListenerForUpdateExercise() {
  document.addEventListener("DOMContentLoaded", () => {
    const button = document.getElementById("updateExerciseButton") as HTMLButtonElement;
    const idInput = document.getElementById("updateIdInput") as HTMLInputElement;
    const nameInput = document.getElementById("updateExerciseNameInput") as HTMLInputElement;
    const descriptionInput = document.getElementById("updateExerciseDescriptionInput") as HTMLInputElement;
...
function updateExercise(id: number, name: string, description: string) {
  fetch(`${import.meta.env.VITE_BASE_API_URL}/exercise/update?id=${id}&name=${name}&description=${description}`, {
    method: 'PUT'})
