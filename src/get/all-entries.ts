...
function generateEntryTable(data: Entry[]) {
    ...
    const headerRow = generateRow(["Id", "Workout Id", "Exercise Id", "Weight", "Sets", "Reps"])
    tblBody.appendChild(headerRow)

    for (let i = 0; i < data.length; i++) {
    const row = generateRow([`${data[i].id}`, `${data[i].workoutId}`, `${data[i].exerciseId}`, `${data[i].weight}`, `${data[i].sets}`, `${data[i].reps}`])
